window.Abschussregeln = (() => {
  ...
  async function init() {
    ...
    jaeger.forEach((person) => el("arJaeger").add(new Option(`${person.vorname} ${person.nachname}`, person.id)));
  ...
  function rendern() {
    ...
    rows.forEach((regel) => {
      ...
      [regel.nr, `${regel.jaeger?.vorname || ""} ${regel.jaeger?.nachname || ""}`.trim(), regel.wildklasse?.bezeichnung || "", typName(regel.regel_typ), regel.frei_ab || "–", regel.originaljahr || "–", regel.freigabejahr || "–", regel.bemerkung || "", regel.aktiv ? "Ja" : "Nein"]
        .forEach((wert, index) => { const td = document.createElement("td"); td.dataset.label = ["Nr.","Jäger","Wildklasse","Regel","Frei ab","Originaljahr","Freigabejahr","Bemerkung","Aktiv"][index]; td.textContent = wert; tr.append(td); });
  ...
  async function neu() {
    ...
    el("arFreiAb").value = ""; el("arOriginaljahr").value = ""; el("arFreigabejahr").value = new Date().getFullYear(); el("arBemerkung").value = ""; el("arAktiv").checked = true; oeffnen();
  }
  function bearbeiten(regel) {
    ...
    el("arOriginaljahr").value = regel.originaljahr || ""; el("arFreigabejahr").value = regel.freigabejahr || ""; el("arBemerkung").value = regel.bemerkung || ""; el("arAktiv").checked = regel.aktiv; oeffnen();
  }
  ...
  async function speichern() {
    const daten = { nr: Number(el("arNr").value), jaeger_id: el("arJaeger").value, wildklasse_id: el("arWildklasse").value, regel_typ: el("arTyp").value,
      frei_ab: el("arFreiAb").value || null, originaljahr: el("arOriginaljahr").value ? Number(el("arOriginaljahr").value) : null,
    ...
    if (!daten.nr || !daten.jaeger_id || !daten.wildklasse_id || !daten.regel_typ || !daten.frei_ab) return AppFeedback.error("Nr., Jäger, Wildklasse, Regel und Frei ab sind erforderlich.");
