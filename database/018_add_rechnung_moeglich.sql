...
begin
  if not exists (
    ...
    where table_schema = 'public' and table_name = 'wildhaendler'
      and column_name = 'rechnung_moeglich'
  ) then
    alter table public.wildhaendler
      add column rechnung_moeglich boolean not null default true;
    update public.wildhaendler
    set rechnung_moeglich = false
    where lower(btrim(coalesce(bezeichnung, ''))) in ('klein', 'klein wildhändler');
  end if;

  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'wildgruppen'
      and column_name = 'rechnung_moeglich'
  ) then
    alter table public.wildgruppen
      add column rechnung_moeglich boolean not null default false;
    update public.wildgruppen
    set rechnung_moeglich = abschussplan;
  end if;
end;
$$;
