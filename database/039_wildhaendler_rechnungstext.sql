...
alter table public.wildhaendler
  add column if not exists rechnungstext text;

comment on column public.wildhaendler.rechnungstext is
  'Optionaler Text für die Bezeichnung neu erzeugter Rechnungspositionen.';

create or replace function public.rechnungsposition_bezeichnung(
  p_rechnungstext text,
  p_wildhaendler_bezeichnung text,
  p_wildgruppe_bezeichnung text
) returns text
language sql
immutable
set search_path = public
as $$
  select coalesce(nullif(btrim(p_rechnungstext), ''), btrim(p_wildhaendler_bezeichnung))
    || ' - ' || btrim(p_wildgruppe_bezeichnung);
$$;
...
declare
  ...
  v_vorlage jsonb;
  v_anzahl integer;
  v_bestehende_beschreibungen jsonb := '{}'::jsonb;
begin
  ...
  if p_rechnung_id is null then
    ...
    select coalesce(jsonb_object_agg(rp.abschuss_id::text, rp.beschreibung), '{}'::jsonb)
      into v_bestehende_beschreibungen
    from public.rechnungspositionen rp
    where rp.rechnung_id = v_id;
  ...
  select
    ...
    coalesce(
      v_bestehende_beschreibungen ->> a.id::text,
      public.rechnungsposition_bezeichnung(
        w.rechnungstext, w.bezeichnung, wg.bezeichnung
      )
    ),
...
grant execute on function public.rechnungsposition_bezeichnung(text, text, text) to authenticated;
